...
contextBridge.exposeInMainWorld('clearweb', {
  getState: () => invoke('app:get-state'), onState: (callback) => ipcRenderer.on('app:state', (_, state) => callback(state)), onProtection: (callback) => ipcRenderer.on('protection:changed', (_, protection) => callback(protection)),
  ...
  getProtectionDetails: () => invoke('protection:get-details'), cleanWithAI: () => invoke('page:clean-ai'),
